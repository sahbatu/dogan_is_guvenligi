...
export function PricesAdminPage() {
  ...
  useEffect(() => {
    ...
    products.forEach((p) => {
      next[p.id] = priceToInputValue(p.price)
    })
    setPrices(next)
    setDirtyIds(new Set())
  }, [products])
  ...
  const setPrice = (id: string, value: string, original: number | null) => {
    setPrices((prev) => ({ ...prev, [id]: value }))
    setSaved(false)
    const parsed = parsePriceInput(value)
    const changed =
      (parsed ?? null) !== (original ?? null) &&
      !(value.trim() === '' && original == null)
    setDirtyIds((prev) => {
  ...
  const handleSave = async () => {
    ...
    for (const id of dirtyIds) {
      updates[id] = parsePriceInput(prices[id] ?? '')
    }

    try {
      if (!isSupabaseConfigured || usingDemo) {
        updateDemoPrices(updates)
      } else {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Tüm ürün fiyatlarını tek ekrandan güncelleyin. Değişiklikleri kaydetmeyi unutmayın.
          </p>
        </div>
      ...
      {usingDemo && (
        <div className="mt-4 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-800">
          Demo mod — fiyatlar tarayıcıda saklanır ve sitede görünür.
        </div>
      )}
      ...
      {saved && (
        <div className="mt-4 rounded-lg bg-green-50 px-4 py-3 text-sm text-green-700">
          {dirtyCount === 0 ? 'Fiyatlar kaydedildi.' : ''}
        </div>
      )}
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-navy-900/5 bg-surface/50">
                ...
                <th className="px-4 py-3 font-semibold text-navy-900">Mevcut</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <Input
                        ...
                        onChange={(e) => setPrice(product.id, e.target.value, product.price)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault()
                            handleSave()
      ...
      {dirtyCount > 0 && (
        <div className="sticky bottom-4 mt-6 flex justify-end">
          <Button size="lg" onClick={handleSave} disabled={saving}>
            <Save className="h-4 w-4" />
            {saving ? 'Kaydediliyor...' : `${dirtyCount} fiyatı kaydet`}
          </Button>
        </div>
      )}
    </div>
